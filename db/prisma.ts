...
// TODO: required
const DATABASE_URL = get('DATABASE_URL').asString();

if (DATABASE_URL?.includes('sslcert=/tmp/ap-northeast-1-bundle.pem')) {
  fs.writeFile(
    `/tmp/ap-northeast-1-bundle.pem`,
    `-----BEGIN CERTIFICATE-----
...
if (!global.prisma) {
  global.prisma = new PrismaClient({
    // datasources: {
    //   db: {
    //     url: `${DATABASE_URL}${DATABASE_URL?.includes('?') ? '&' : '?'}connection_limit=1&pool_timeout=0&statement_cache_size=50`,
    //   },
    // },
